/*
	...
	https://dronebotworkshop.com/mpu-6050-level/#Electronic_Level_Sketch
*/
...
float angle_pitch_output, angle_roll_output, angle_roll_display;
...
void setup() {
	...
	char buffer[40];
	sprintf(buffer, "x:%d, y:%d, z:%d", gyro_x_cal, gyro_y_cal, gyro_z_cal);
...
void loop(){
	...
	if (displaycount > 100) {
		// my mpu5060 is mounted wrong
		angle_roll_display = angle_roll_output + 90;
 
		// Print to Serial Monitor	 
		char buffer[40];
		sprintf(buffer, "x:%d, y:%d, z:%d", gyro_x, gyro_y, gyro_z);
		Serial.println(buffer);

		char sroll[10];
		char spitch[10];
		dtostrf(angle_pitch_output, 6, 3, spitch);
		dtostrf(angle_roll_display, 6, 3, sroll);
		sprintf(buffer, "Pitch:%s, Roll:%s", spitch, sroll);
		Serial.println(buffer);
		//Serial.print("Pitch:"); 
		//Serial.print(angle_pitch_output);
		//Serial.print(", Roll:"); 
		//Serial.println( angle_roll_display); 
		displaycount = 0;
	}
